...
public class Event {
    ...
    private static Event genericAlarmEvent(String node) {
        return new Event()
                ...
                .addParm(new Event.Parameter("node", Objects.requireNonNull(node)));
    }
    ...
    private static Event createGenericSituationAlarmEvent(List<String> nodes) {
        ...
        Event event = new Event()
                .setUei("uei.opennms.org/alarms/trigger")
                .addParm(new Event.Parameter("node", "situation"));

        for (int i = 0; i < nodes.size(); i++) {
            event.addParm(new Event.Parameter("related-reductionKey" + (i + 1),
